...
class Kohana_Route {
	...
	public function uri(array $params = NULL)
	{
		...
		list($uri) = $this->uri_compile($this->_uri, TRUE, $defaults, $params);
	...
	private function uri_compile($portion, $required, $defaults, $params)
		{
			$missing = array();

			$pattern = '#(?:'.Route::REGEX_KEY.'|'.Route::REGEX_GROUP.')#';
			$result = preg_replace_callback($pattern, call_user_func_array(array($this, 'uri_compile_replace'), array($matches, $defaults, $missing, $params, $required)), $portion);
	...
	private function uri_compile_replace($matches, $defaults, &$missing, $params, &$required)
		{
			if ($matches[0][0] === '<')
			{
				// Parameter, unwrapped
			...
			{
				// Group, unwrapped
				$result = $this->uri_compile($matches[2], FALSE, $defaults, $params);
